...
## Free identification with Google Gemini (no credit card)

If you don't want to pay OpenAI for identification, use Google's free tier:
1. Go to **aistudio.google.com** (sign in with Google) → **Get API key** → create a key. No card required.
2. In Netlify → Environment variables, add **`GEMINI_API_KEY`** = that key. Optional `GEMINI_MODEL` (default `gemini-2.0-flash`; set to the current free Flash model if that name changes).
3. Redeploy.

Engine priority: iNaturalist (if set) → **Gemini (free)** → OpenAI (paid) → Google Vision → on-device. So with only a Gemini key, identification runs free. (AI illustrations still use OpenAI and are opt-in.)
